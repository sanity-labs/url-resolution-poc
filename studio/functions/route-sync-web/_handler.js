    ...
    return documentEventHandler(async ({ context, event }) => {
        ...
        if (parentRoutes.length > 0) {
            ...
            for (const route of parentRoutes) {
                ...
                const children = await client.fetch(`*[_type in [${typeFilter}] && references($parentId)]{ _id, _type }`, { parentId: docId });
                for (const child of children || []) {
                    await syncSingleDocument(client, route, channel, child._id, child._type);
                }
                console.log(`[@sanity/routes] Re-synced ${(children || []).length} child document(s) for parent ${docId}`);
            }
            return;
        }
